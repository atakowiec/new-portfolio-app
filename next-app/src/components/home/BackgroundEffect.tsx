...
import {getColorBetween, random, randomPointOutsideRect} from "@/util/util";
...
export default function BackgroundEffect() {
  ...
  useEffect(() => {
    const onResize = () => {
      ...
      setNodeCount(Math.round((width * height) / 168000));
    }
...
function Node({containerRef, index}: { containerRef: RefObject<HTMLDivElement | null>, index: number }) {
  ...
  useEffect(() => {
    ...
    function updatePath() {
      ...
      setPath(path)
      setColor(getColorBetween("#b676f3", "#5c189f"))
